...
<body>
	...
	<div id="tudo">
		...
		<form action="processa_cad_atorFilme.php" method="post">
		<select name="selectFilme">
					<option>Selecione o filme</option>
						<?php
	                        require_once ("conn.php");
							$seleciona = "SELECT * FROM tbfilme";
							$selecionaTipo = mysqli_query($conn, $seleciona);
							while($row_tipo = mysqli_fetch_assoc($selecionaTipo)){ ?>
								<option value="<?php echo $row_tipo['codFilme']; ?>"><?php echo $row_tipo['nomeFilme']; ?></option> <?php
						}
					?>
		</select><br> <br>
        <select name="selectAtor">
					<option>Selecione o Ator do Filme</option>
						<?php
	                        require_once ("conn.php");
							$seleciona = "SELECT * FROM tbator";
							$selecionaTipo = mysqli_query($conn, $seleciona);
							while($row_tipo = mysqli_fetch_assoc($selecionaTipo)){ ?>
								<option value="<?php echo $row_tipo['codAtor']; ?>"><?php echo $row_tipo['nomeAtor']; ?></option> <?php
						}
					?>
		</select><br><br>
